...
function addRow() {
    ...
    let rowNode = $('#createRequisition').DataTable()
        .row.add([row,
            ...
            `<textarea id="item_comentario_${row}" class="form-control"></textarea>`,
...
function saveRequisition(action = null) {
    ...
    let comment = $("#commentPrincipal").val();



    if (!comment) {
        messageAlert("Debe escriber el comentario principal.", "warning");
        return;
    }
    ...
    for (const key in items) {
        ...
        formdata.append("item_comentario_" + i, $("#item_comentario_" + items[key]).val());
        i++;
    }
    ...
    $.ajax({
                ...
                success: function(data) {
                        ...
                        for (const key in data.detailRequisition) {
                            ...
                            switch (status) {
                                case 'Procesada':
                                    ...
                                    break;
                                case 'Cotizada':
                                    if (data.userDireccion != true) {
                                        isValid = true;
                                    }
                                    break;
                                case 'Aprobada':
                                    if (data.userFinanzas != true) {
                                        isValid = true;
                                        console.log("finanzas modifica");
                                    }
                                    break;
                                case 'Pagada':
                                    isValid = true;
                                ...
                                case 'Entregada':
                                    ...
                                    break;
                                case 'Cancelada':
                                    if (data.userCompras == true || data.userDireccion || data.userFinanzas) {
                                        isValid = true;
                                    }
                                    break;
                                case 'Aprobada':
                            ...
                            switch (status) {
                                ...
                                case "Procesada":
                                    ...
                                    break;
                                case "Cotizada":
                                    statusOptionItem = '<option value="Cotizada" selected>Cotizada</option><option value="Aprobada">Aprobada</option><option value="Cancelada">Cancelada</option>';
                                    break;
                                case "Aprobada":
                                    statusOptionItem = '<option value="Aprobada">Aprobada</option>';
                                    break;
                                case "Pagada":
                                    statusOptionItem = '<option value="Pagada">Pagada</option>';
                                    break;
                                case "Entregada":
                                    statusOptionItem = '<option value="Entregada">Entregada</option>';
                                    break;
                        ...
                        <input type="text" disabled class="form-control" value="${data.detailRequisition[key].num_item}">`,
                                        ...
                                        `<textarea ${(isValid) ? 'disabled' : ''} id="item_comentario_${row}" class="form-control">${data.detailRequisition[key].comment}</textarea>`,
                        ...
                        <button type="button" class="btn btn-secondary" data-toggle="tooltip" data-placement="top" title="Archivos" onclick="showModalFileItem(${row}, ${id})"><i class="fas fa-list"></i></button>
                        ${(data.permission === true) ? `<span ${(status === 'Cancelada') ? 'style="display:none"' : 'style="display:block"'} data-toggle='modal' data-target='#modalProvider' data-backdrop='static'><button class='btn btn-primary' onclick='showProvider(${data.detailRequisition[key].id},${data.detailRequisition[key].quantity})' data-toggle='tooltip' data-placement='top' title='Agregar Proveedores'><i class='fas fa-box' /></button></span>` : ''}</div>`
                    ])
                    .draw()
                    .node();
    ...
    $.ajax({
        ...
        success: function(data) {
            ...
            if (data.error == true) {
                ...
                var url = "{{asset('')}}";
                console.log(data);
                if (data.userfinanzas == true && data.totalFacturas <= 0) {
                    $("#divFactura").show();
                }
...
function showModalFileItem(row, requi) {
    let idItem = $(`#item_id_${row}`).val();
    $("#hiddeIditem").val(idItem);
    $("#hiddeIdRequisitionitem").val(requi);
    $("#modalCreateRequisition").modal('hide');
    $("#modalFilesRequisitionItems").modal('show');
    $("#divFacturaItem").hide();
    $("#divEntregado").hide();
    $("#divArchivosItem").show();
    ...
    $.ajax({
        ...
        type: "GET",
        url: `requisitions/${idItem}/filesItems`,
        data: {"requi": requi},
        /*
        cache: false,
        contentType: false,
        ...
        success: function(data) {
            ...
            if (data.error == true) {
                ...
                if (data.userfinanzas == true && data.totalFacturas <= 0 && data.subirFactura == true) {
                    $("#divFacturaItem").show();
                }
                console.log(data.totalEntregado);
                if (data.userCompras == true && data.totalEntregado <= 0 && data.subirEntregado == true) {
                    $("#divEntregado").show();
                }
                if(data.statusRequi =="Entregada"){
                    $("#divArchivosItem").hide();
                }
                $("#bodyFilesItem").empty();

                for (const key in data.requisitionFilesItem) {
                    html += `<tr>` +
                        `<td>${data.requisitionFilesItem[key].id}</td>` +
                        `<td><a href="${data.requisitionFilesItem[key].ruta}/${data.requisitionFilesItem[key].name}" target="_blank">${data.requisitionFilesItem[key].name}</a></td>`+
                        `<td><textarea name ="txt_comentario_${data.requisitionFilesItem[key].id}" id="txt_comentario_${data.requisitionFilesItem[key].id}">${data.requisitionFilesItem[key].comment}</textarea></td>`+
                        `<td>`;
                    if (data.userCompras == true && data.statusRequi !="Entregada") {
                        html += `<span>`+
                        `<button type="button" class="btn btn-primary" data-toggle="tooltip" data-placement="top" title="Guardar Comentario" onclick="saveComment(${data.requisitionFilesItem[key].id})">`+
                            `<i class="fas fa-check"></i>`+
                        `</button>`+
                        `</span>`;
                    }
                    if (data.userAdmin == true) {
                        html += `<span >` +
                            `<button type="button" class="btn btn-danger" data-toggle="tooltip" data-placement="top" title="Cancelar" onclick="deleteFile(${data.requisitionFilesItem[key].id})">` +
                            `<i class="fas fa-minus-square"></i>` +
                            `</button>` +
                            `</span>`;
                    }
...
function uploadFilesItem(tipo) {

    let file;
    if(tipo == "item"){
        file = $('#inputFileItem')[0];
        item = $("#hiddeIditem").val();
        requi = $("#hiddeIdRequisitionitem").val();
        console.log(requi+': '+ item);
    }else if(tipo == "factura"){
        file = $('#inputFileFacturaItem')[0];
        item = $("#hiddeIditem").val();
        requi = $("#hiddeIdRequisitionitem").val();
        console.log(requi+': '+ item);
    }else if(tipo == "entregado"){
        file = $('#inputFileEntregadoItem')[0];
        item = $("#hiddeIditem").val();
        requi = $("#hiddeIdRequisitionitem").val();
        console.log(requi+': '+ item);
    }
    ...
    $.ajax({
        ...
        type: "POST",
        url: `requisitions/${item}/uploadFileItem`,
        data: data,
        cache: false,
        contentType: false,
        processData: false,
    ...
    Swal.fire({
        ...
        if (result.value != '') {
            $.ajax({
                ...
                type: "POST",
                url: `requisitions/${id}/updateStatusRequisition`,
                data: { "status": status, 'comentario':result.value },
                /*cache: false,
                contentType: false,
                processData: false,*/
                dataType: 'json',
          ...
          //Swal.fire('Se regreso la requisicion', '', 'success')
        } else {
          Swal.fire('Debe agregar un comentario', '', 'info')
        }
      })
    ...
    $.ajax({
        ...
        success: function(data) {
            ...
            for (const key in data.requisitionHistory) {
                ...
                html+= `<span>`;
                if (data.requisitionHistory[key].comment != null) {
                    console.log(data.requisitionHistory[key].comment.split(","));
                    arrayCadena = data.requisitionHistory[key].comment.split(",")
                    if(arrayCadena[0] != ""){
                        html+= `${data.requisitionHistory[key].status} (${data.requisitionHistory[key].comment}) /`;
                    }else{
                        html+= `${data.requisitionHistory[key].status} /`;
                    }
                }else{{
                    html+= `${data.requisitionHistory[key].status} /`;
                }}
                html+= ` ${data.requisitionHistory[key].created_at} / ${data.requisitionHistory[key].name} ${data.requisitionHistory[key].last_name} / ${data.requisitionHistory[key].comment}.</span>`;
                html+= `</li>`;
            }
            $('#step').append(html);
            console.log(items);
...
function listaRequisiciones(tipo) {
    console.log(tipo);
    $("#tableRequisitions").hide();
    $("#tableRequisitionsCanceladas").show();
    $.ajax({
        headers: {
            'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
        },
        type: "GET",
        url: `requisitions/lista`,
        data: {"tipo":tipo},
        dataType: 'json',
        success: function(data) {
            console.log(data);
            $("#tableRequisitionsCanceladas").DataTable({
                dom: 'Bfrtip',
                paging: false,
                destroy: true,
                order:[[1,"desc"]],
                data: data.requisiciones,
                columns: [
                    { data: 'id' },
                    { data: 'name' },
                    { data: 'area' },
                ...
                columnDefs: [
                    {
                        ...
                        render: function (data, type, row, meta) {
                            ...
                            <span data-toggle="modal" data-target="#modalHistorial">
                                <button type="button" class="btn btn-warning" data-toggle="tooltip" data-placement="top" title="Historial" onclick="history(${row.id})">
