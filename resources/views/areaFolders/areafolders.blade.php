                ...
                -->
                    @switch($area)
                        @case('almacen')
                            ...
                            @break
                        @case('calidad')
                            @if (Auth::user()->hasAnyRole(['calidad', 'admin', 'coordinador de calidad']))
                                <button type="button" class="btn btn-primary" onclick="newFolder({{ $folders[0]['area_id'] }}, 0)">
                                    Agregar carpeta en el primer nivel
                                </button>
                        ...
                        @case('operaciones')
                            ...
                            @break
                        @case('compras')
                            @if (Auth::user()->hasAnyRole(['compras', 'admin', 'finanzas', 'coordinador de finanazas']))
                                <button type="button" class="btn btn-primary" onclick="newFolder({{ $folders[0]['area_id'] }}, 0)">
                                    Agregar carpeta en el primer nivel
                                </button>
                        ...
                        @case('direccion')
                            ...
                            @break
                        @case('finanzas')
                            @if (Auth::user()->hasAnyRole(['finanzas', 'admin', 'coordinador de finanzas', 'direccion']))
                                <button type="button" class="btn btn-primary" onclick="newFolder({{ $folders[0]['area_id'] }}, 0)">
                                    Agregar carpeta en el primer nivel
                                </button>
                            ...
                            @break
                        @case('ingenieria')
                            @if (Auth::user()->hasAnyRole(['ingenieria', 'admin', 'coordinador operacional', 'direccion']))
                                <button type="button" class="btn btn-primary" onclick="newFolder({{ $folders[0]['area_id'] }}, 0)">
                                    Agregar carpeta en el primer nivel
                                </button>
                        ...
                        @case('recursos humanos')
                            ...
                            @break
                        @case('ventas')
                            @if (Auth::user()->hasAnyRole(['ventas', 'admin', 'finanzas', 'coordinador comercial', 'direccion']))
                                <button type="button" class="btn btn-primary" onclick="newFolder({{ $folders[0]['area_id'] }}, 0)">
                                    Agregar carpeta en el primer nivel
                                </button>
            ...
            <div class="card">
                <div class="card-body">
                    ...
                    @if(!isset($folders[0]['empty']))
                        @switch($area)
                            @case('finanzas')
                                @if (Auth::user()->hasAnyRole(['finanzas', 'admin', 'calidad', 'direccion', 'coordinador de finanzas', 'coordinador operacional']))
                                ...
                                @break
                            @case('ventas')
                                @if (Auth::user()->hasAnyRole(['ventas', 'admin', 'ingenieria', 'finanzas', 'operaciones', 'servicio', 'calidad', 'pruebas', 'coordinador comercial', 'coordinador operacional', 'direccion']))
                                    <select id="selectNivel{{ $folders[0]['nivel'] }}" class="form-control" onchange="getFoldersAndFiles({{ $folders[0]['area_id'] }}, {{ $folders[0]['nivel'] }})">
                                        <option value="">Seleccione</option>
                                        @foreach($folders as $folder)
                                        <option value="{{ $folder['id'] }}">{{ $folder['name'] }}</option>
                                ...
                                @break
                            @case('ingenieria')
                                @if (Auth::user()->hasAnyRole(['ingenieria', 'admin', 'manufactura', 'almacen', 'compras', 'operaciones', 'servicio', 'calidad', 'pruebas', 'direccion', 'coordinador operacional', 'coordinador de finanzas', 'coordinador de calidad', 'coordinador comercial']))
                                    <select id="selectNivel{{ $folders[0]['nivel'] }}" class="form-control" onchange="getFoldersAndFiles({{ $folders[0]['area_id'] }}, {{ $folders[0]['nivel'] }})">
                                        <option value="">Seleccione</option>
                                        @foreach($folders as $folder)
                                        <option value="{{ $folder['id'] }}">{{ $folder['name'] }}</option>
                                ...
                                @break
                            @case('compras')
                                @if (Auth::user()->hasAnyRole(['compras', 'admin', 'finanzas', 'operaciones', 'calidad', 'pruebas', 'direccion', 'coordinador operacional', 'coordinador comercial', 'coordinador de finanzas', 'coordinador de calidad']))
                                    <select id="selectNivel{{ $folders[0]['nivel'] }}" class="form-control" onchange="getFoldersAndFiles({{ $folders[0]['area_id'] }}, {{ $folders[0]['nivel'] }})">
                                        <option value="">Seleccione</option>
                                        @foreach($folders as $folder)
                                        <option value="{{ $folder['id'] }}">{{ $folder['name'] }}</option>
                                ...
                                @if (Auth::user()->hasAnyRole(['compras', 'admin', 'finanzas', 'coordinador de finanzas']))
                                    <button id="btnLevel1" type="button" class="btn btn-primary form-button" onclick="newFolder({{ $folders[0]['area_id'] }}, {{ $folders[0]['nivel'] }})"
                                    style="display:none;">
                                    Agregar carpeta</button>
                                    <button id="btnLevelModify1" type="button" class="btn btn-info form-button" onclick="cambiaNombreFolder({{$folders[0]['id']}})"
